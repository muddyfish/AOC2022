...
def get_deltas_at_time(time: int, position):
    ...
    for dy, dx in MOVE_DIRECTIONS:
        if position == INITIAL_POS and (dy, dx) not in ((1, 0), (0, 0)):
            continue
        if position == END_POS and (dy, dx) not in ((-1, 0), (0, 0)):
            continue
...
begin = run_a_star(INITIAL_POS, END_POS, 0)
end = run_a_star(END_POS, INITIAL_POS, begin)
final = run_a_star(INITIAL_POS, END_POS, end)
print(final)
